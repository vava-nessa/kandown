 ...
 *
 * @exports DependencyGateError, resolveTransition, assertTransitionAllowed,
...
export class DependencyGateError extends Error {
  constructor(
    ...
    const list = blockedBy.length === 1
      ? blockedBy[0]
      : `${blockedBy.slice(0, -1).join(', ')} and ${blockedBy[blocked.length - 1]}`;
...
export function isTerminalStatus(status: string, config: Pick<KandownConfig, 'board'> = DEFAULT_CONFIG): boolean {
  return status === terminalStatus(config) || isArchivedStatus(status);
}
...
function normalizeDeps(task: TransitionTaskInput, taskId: string): string[] {
...
export function resolveDependencyStatus(
  tasks: Iterable<TransitionTaskInput>,
  config: Pick<KandownConfig, 'board'> = DEFAULT_CONFIG,
): Map<string, { exists: boolean; resolved: boolean; title: string | null }> {
  const byId = new Map<string, TransitionTaskInput>();
  for (const t of tasks) {
    const id = (t && (t as { id?: unknown }).id) as string | undefined;
    if (id) byId.set(id, t);
  }
  ...
  for (const [id, task] of byId) {
    const status = typeof task.status === 'string' ? task.status.toLowerCase() : 'backlog';
    const isArch = isArchivedStatus(task);
    out.set(id, {
      exists: true,
      resolved: isArch || status === terminal,
      title: null,
    });
  }
  for (const task of byId.values()) {
    const deps = normalizeDeps(task, (task.id as string) ?? '');
...
export function unresolvedDependencyIds(
  ...
  const id = (task && (task as { id?: string }).id) || task.frontmatter?.id || '';
...
export function resolveTransition(
  ...
  if (typeof targetStatus !== 'string' || !targetStatus) {
    return { allowed: false, reason: 'not-implemented', blockedBy: [] };
  }
...
export function assertTransitionAllowed(
  ...
  if (!verdict.allowed) {
    throw new DependencyGateError(
      ((task && (task as { id?: string }).id) || task.frontmatter?.id || ''),
      targetStatus,
      verdict.blockedBy,
      verdict.reason,
    );
