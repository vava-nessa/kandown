...
- **`App.tsx`** · 330 lines — Composes the Kandown web UI, hydrates recent projects from the URL, and owns global keyboard shortcuts for board navigation, creation, reload, search focus, and command palette access.
...
- **`board-reader.ts`** · 743 lines — Provides filesystem-based reading and writing of Kandown task files for the CLI.
...
- **`model-catalog.spec.ts`** · 116 lines — Locks the parts of listHarnessModels that must hold on every machine: harnesses that do not speak ACP (and unknown ids) answer the curated baseline with source "baseline", ids are normalized entries, and the cache reset clears state.
...
- **`model-catalog.ts`** · 372 lines — Answers the chat model menu's question, "what can I pick for this harness?", with real model ids instead of a cosmetic shortlist.
...
- **`settings.tsx`** · 529 lines — Interactive settings editor for kandown.json.
...
- **`Column.tsx`** · 368 lines — Renders a single kanban column, accepts dropped cards, shows the filtered task count, and creates new tasks directly in the column.
...
- **`CommandPalette.tsx`** · 479 lines — Provides global quick actions, page navigation, view switching, task lookup, and content-aware task search with highlighted snippets, plus a Conversations group that resumes the project's recent agent chat sessions.
...
- **`ConversationSection.tsx`** · 374 lines — The project's agent chat sessions under the rail's nav items: a header row with a "+ new chat" button, a compact filter input, recency groups (Today / Yesterday / Last 7 days / Older) and one row per conversation.
...
- **`Header.tsx`** · 515 lines — Top navigation bar for project switching, task search, filters (including the category dropdown next to the task count), view mode, density, settings, command palette, reload, and task creation.
...
- **`SideNav.tsx`** · 377 lines — Collapsed-by-default icon rail that expands to a full sidebar on click.
...
- **`TaskHoverCard.tsx`** · 434 lines — Round 11 of vava's chat feedback: task mentions in the agent chat stay clickable AND gain a hover preview.
...
- **`ContextCards.tsx`** · 186 lines — Faithful port of the BeautifulUI ContextCards (beautifului.dev, MIT): retrieved chunks enter once with a staggered fade-up, then remain available; source chips pop in after a beat.
...
- **`PromptBar.tsx`** · 1072 lines — Faithful port of the official BeautifulUI PromptBar (beautifului.dev, MIT): a composer with real controls, an attach menu, @ data-source and / command menus, a model picker, dictation and send.
...
- **`TaskRows.tsx`** · 276 lines — Faithful port of the BeautifulUI TaskRows component (beautifului.dev, MIT): a progress list where each row carries a spinner ring or a success/failure badge plus an expandable details grid.
...
- **`ToolChips.tsx`** · 376 lines — An agent run as compact tool rows with inline chips, then file-diff chips summarizing the edits; hovering a file chip opens its diff preview in a body portal.
...
- **`agent-chat-events.ts`** · 530 lines — Pure, framework-free model of the JSON events the daemon streams on GET /api/agent/sessions/:id/events, plus the reducer that folds them into the chat message list rendered by the agent chat sidebar.
...
- **`demoBackend.ts`** · 397 lines — Implements the Kandown REST API against a `Map` instead of a disk, so the whole web UI can run in a browser tab with no CLI, no server and no storage.
...
- **`filesystem.ts`** · 1715 lines — Wraps the File System Access API, project discovery, task reads and writes, project config persistence, and recent-project IndexedDB storage.
...
- **`store.ts`** · 2269 lines — Central state container for project handles, task-derived board data, config, filters, task drawer editing, content-search cache, recent projects, toast notifications, and watcher-driven browser/audio notifications.
...
- **`agent-chat-events.spec.ts`** · 314 lines — Locks the reducer contract the chat sidebar renders from: streaming text accumulates then finalizes, thinking stays on its own channel, tool calls pair by toolCallId, usage accumulates, errors append entries (and fatal ones close the…
...
- **`agent-runs.spec.ts`** · 371 lines — Covers the pure core exported by src/lib/store/agentRunsSlice.ts (most-active-run pick, badge tone mapping, terminal-state set, PTY panel decisions, wire guards) and the transport behavior of the slice actions (poll merge with stable…
...
- **`category-color.spec.ts`** · 73 lines — Verifies the deterministic contract of src/lib/category-color.ts: same category string always yields the same color and icon, the foreground is always a legible dark or white on the chosen background, and the palette covers the full…
...
- **`markdown-stream.spec.ts`** · 77 lines — Locks the two pure helpers that keep a streaming assistant message presentable: `closeDanglingCodeFence` (an odd number of ``` markers means the stream cut a fenced block open, so a closing fence is appended before the parser sees the…
...
- **`projectSlice.ts`** · 392 lines — Opening a project (file-picker or recent), server-mode auto-open, config load/update, and the full board reload.
- **`types.ts`** · 595 lines — State shape and supporting interfaces used across every store slice.
...
- **`base.ts`** · 111 lines — The only bundled theme since the UI redesign.
- **`index.ts`** · 31 lines — Aggregates the bundled theme presets into the THEME_PRESETS array consumed by the theme engine (src/lib/theme.ts).
...
332 of 332 eligible files carry an `@description` header.
