...
- **`App.tsx`** · 241 lines — Composes the Kandown web UI, hydrates recent projects from the URL, and owns global keyboard shortcuts for board navigation, creation, reload, search focus, and command palette access.
- **`main.tsx`** · 37 lines — Mounts the Kandown React app into the Vite-provided root node, initializes i18n, and imports the global Tailwind/CSS-variable theme layer.
...
- **`cli.ts`** · 172 lines — Entrypoint for the kandown command line tool.
...
- **`tasks.ts`** · 301 lines — Handlers for `kandown list/show/create/move/assign/commit` plus `export`/`import`/`projects`.
...
- **`use-mouse.ts`** · 128 lines — Provides terminal mouse tracking enable/disable and a parser for SGR mouse sequences.

## `src/cli/lib/` — CLI core — daemon, server, config, board access, MCP

- **`agents.ts`** · 217 lines — Defines all supported AI coding agents, detects which are installed on the current system, and builds the shell command to launch each one with a task context + system prompt.
...
- **`mcp.ts`** · 236 lines — Exposes Kandown task operations to MCP hosts (Claude Desktop, VSCode, etc.) via JSON-RPC 2.0 over stdin/stdout.
...
- **`board.tsx`** · 1204 lines — Interactive kanban board for the Kandown CLI.
...
- **`Header.tsx`** · 425 lines — Top navigation bar for project switching, task search, filters, view mode, density, settings, command palette, reload, and task creation.
...
- **`OnboardingTour.tsx`** · 111 lines — Quick guide on first launch introducing Kandown features.
...
- **`filesystem.ts`** · 879 lines — Wraps the File System Access API, project discovery, task reads and writes, project config persistence, and recent-project IndexedDB storage.
...
- **`parser.ts`** · 371 lines — Parses Kandown task markdown into typed structures, builds board columns from task frontmatter, extracts editable subtasks, reinjects them on save, and performs lightweight cached task-content search for the web UI.
...
- **`boardSlice.ts`** · 571 lines — Column CRUD, task move/reorder/create/delete/archive, task selection, and bulk operations.
...
- **`watcherSlice.ts`** · 223 lines — Starts either the browser file-system watcher or the server-mode REST polling loop, translates watcher events into board reloads / conflict detection / notifications, and exposes a manual restart for when the watcher auto-disables…
...
146 of 146 eligible files carry an `@description` header.
