  ...
  for (const dir of model.dirs) {
    ...
    for (const file of model.files.filter(f => f.dir === dir)) {
      ...
      if (file.generatedFrom) {
        ...
        // 📖 No line count for generated files: their size is build churn
        // (a rebuild on any machine changes it), which made the committed map
        // drift from CI's own render and fail the check on every release.
        out.push(`- **\`${name}\`** · ⚠️ **GENERATED** by ${by} — edit \`${source}\` instead`);
      } else {
  ...
  return JSON.stringify(
    {
      ...
      files: model.files.map(f => ({
        path: f.path,
        ...(f.generatedFrom
          ? { generated: true, source: f.generatedFrom.source, generatedBy: f.generatedFrom.by }
          : { lines: f.lines }),
