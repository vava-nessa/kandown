...
import type { TransitionTaskInput } from '../dependencies';
import {
  resolveTransition,
...
import type { ParsedTask, KandownConfig } from '../types';
...
function pt(id: string, status: string = 'Backlog', deps: unknown = [], archived: boolean | string = false): ParsedTask {
  ...
  return {
    frontmatter: {
      ...
      archived,
    } as unknown as ParsedTask['frontmatter'],
    body: '',
  };
}
...
function board(...tasks: ParsedTask[]): Map<string, { exists: boolean; resolved: boolean }> {
  ...
  return map as Map<string, { exists: boolean; resolved: boolean }>;
}
...
describe('resolveTransition — happy path', () => {
  it('allows a free move between non-terminal columns', () => {
    ...
    expect(resolveTransition(pt('t1', 'Backlog'), 'Todo', snap, config)).toEqual({
      allowed: true,
  ...
  it('allows terminal when nothing blocks', () => {
    ...
    expect(resolveTransition(pt('t1', 'In Progress'), 'Done', snap, config)).toEqual({
      allowed: true,
      reason: 'allowed',
    });
  });
  it('allows archiving when nothing blocks', () => {
    const snap = board(pt('t1', 'In Progress'));
    expect(resolveTransition(pt('t1', 'In Progress'), 'archived', snap, config)).toEqual({
      allowed: true,
      reason: 'allowed',
    });
  });
...
describe('resolveTransition — resolution rules', () => {
  ...
  it('survives a stringified depends_on', () => {
    const t1: ParsedTask = {
      frontmatter: { id: 't1', title: 't1', status: 'In Progress', depends_on: 't2' } as unknown as ParsedTask['frontmatter'],
      body: '',
    };
    const t2 = pt('t2', 'Done');
    const snap = board(t1, t2);
  ...
  it('survives a junk depends_on', () => {
    const t1: ParsedTask = {
      frontmatter: { id: 't1', title: 't1', status: 'In Progress', depends_on: ['', null, ' ', 't2'] as unknown as string[] } as unknown as ParsedTask['frontmatter'],
      body: '',
    };
    const t2 = pt('t2', 'Backlog');
    const snap = board(t1, t2);
...
describe('assertTransitionAllowed', () => {
  it('returns silently when allowed', () => {
    const snap = board(pt('t1', 'In Progress'));
    expect(() => assertTransitionAllowed(pt('t1', 'In Progress'), 'Done', snap, config)).not.toThrow();
  });
...
describe('terminalStatus / dependency helpers', () => {
  ...
  it('falls back to "Done" when columns are missing', () => {
    const empty = { board: { columns: [] } } as unknown as KandownConfig;
