/**
 * @file Sidebar nav from BeautifulUI
 * @description Faithful port of the BeautifulUI SidebarNav
 * (beautifului.dev, MIT): compact workspace switcher, primary navigation,
 * searchable chat history, and a collapse that preserves icon alignment.
 * The @central-icons-react icons of the original are swapped for the
 * closest @tabler/icons-react equivalents at the same size. The collapse
 * animation is driven by the .sidebar-* rules appended to
 * styles/beautifului.css.
 *
 * @exports SidebarNav
 * @exports SidebarRecent
 * @see src/components/bui/primitives/GlideMenu.tsx
...
const NAV_ITEMS = [
  ...
  { key: "invite", label: "Invite users", icon: <IconUserPlus size={18} />, count: "3/10" },
];
    ...
    >
      <GlideMenu className="flex flex-col gap-px" highlightClassName="inset-x-0 rounded-[8px] bg-hover-2">
        ...
        >
          ...
          <span className="shrink-0 text-ink"><IconCheck size={18} /></span>
        </button>
        <div className="my-1 h-px bg-line" />
        {[
          { label: "New workspace", icon: <IconPlus size={16} /> },
          { label: "Workspace settings", icon: <IconSettings size={16} /> },
          { label: "Invite team members", icon: <IconUserPlus size={16} /> },
        ].map((item) => (
          <button
            key={item.label}
            data-menu-row
        ...
        <button
          ...
          className="relative z-10 flex h-9 w-full items-center gap-1.5 rounded-[8px] px-2 text-left"
        >
          <span className="flex size-5 shrink-0 items-center justify-center text-ink-2"><IconLogout size={16} /></span>
